...
class ProjectSubmissionView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        analysis_result = analyzer.check_plagiarism_and_suggest_features(
            ...
            existing_abstracts=archived_abstracts # Pass the text directly
        )
        
        # --- 7. FINAL DECISION (The Guaranteed Gatekeeper) ---
        if analysis_result['originality_status'] == "BLOCKED_HIGH_SIMILARITY" and analysis_result['feasibility'] < 5.0:
            return Response({
                "detail": "Submission Blocked: High Similarity Detected and Feasibility is Low. Please revise your idea.",
...
class StudentDashboardView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        # Filter submissions by the current authenticated user
...
class LeaderboardView(generics.ListAPIView):
    ...
    def get_queryset(self):
        # This will need more advanced logic, but for now, we'll find students with completed projects
        # and order them by the most innovative
        return User.objects.filter(projects__status='Completed').annotate(
            total_innovation=Sum('projects__submission__innovation_score')
...
class AlumniPortalView(generics.ListAPIView):
    ...
    def get_queryset(self):
        # Get all projects that are completed or archived for the authenticated user
...
class ProjectProgressView(APIView):
    ...
    def get(self, request, project_id, *args, **kwargs):
        try:
            # We fetch the Project model which contains the progress_percentage field
            project = Project.objects.get(id=project_id)
...
class ProgressUpdateView(APIView):
    ...
    def patch(self, request, project_id, *args, **kwargs):
        ...
        # 3. Ownership Check: Only the submitter (or a team member) can update progress
        # Assuming the submitter is the only team member for now
