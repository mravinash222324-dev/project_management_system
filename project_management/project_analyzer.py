...
class ProjectAnalyzer:
    ...
    def check_plagiarism_and_suggest_features(self, title, abstract, existing_abstracts):
        """
        AI Gatekeeper: Performs semantic similarity check and suggests features using LLM reasoning.
        This bypasses the error-prone local vector math.
        """
        highest_similarity = 0.0
        
        # Step 1: Check against existing abstracts using LLM reasoning
        if existing_abstracts:
            
            archived_text = "\n---\n".join(existing_abstracts)

            # --- CRITICAL AI REASONING PROMPT (The new Plagiarism Check) ---
            similarity_prompt = f"""
            You are a semantic analysis engine. A new project idea has been submitted. 
            
            **NEW IDEA:** "{abstract}"
            
            **ARCHIVED IDEAS (Previous Submissions):**
            {archived_text}
            
            Your task is to calculate the conceptual similarity score between the NEW IDEA and the ARCHIVED IDEAS (0.00 to 1.00). Focus on the core function, not keywords.
            
            Respond ONLY with the single highest similarity score you found (e.g., 0.92) as a number. If no similarity is found, respond 0.00.
            """
            
            try:
                response = self.llm_model.generate_content(similarity_prompt)
                score_match = re.search(r'(\d+\.\d+)', response.text) # Match any decimal number
                if score_match:
        ...
        # Step 3: Combined Analysis Prompt (for scores and final report)
        analysis_prompt = f"""
        ...
        try:
            ...
            # --- FINAL SCORING PARSING (Robust Regex) ---
